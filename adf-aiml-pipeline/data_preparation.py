...
def create_index(config, credential, form_recognizer_client=None, embedding_model_endpoint=None, use_layout=False, njobs=4):
    ...
    if not create_or_update_search_index(service_name, subscription_id, resource_group, index_name, config["semantic_config_name"], credential, language,  config["vector_config_name"], admin_key=admin_key):
    ...
    for data_config in data_configs:
        ...
        folder = "C:\\Users\\hifazhassan\\OneDrive - Microsoft\\projects\\mock_data\\test_openai_docs\\chunks\\"
        for chunk_details in result.chunks:
           chunk_id = json.loads(chunk_details.metadata).get("chunk_id", None)
           if chunk_id is not None:
            file_name = chunk_details.filepath
            file_name_without_extension, file_extension = os.path.splitext(file_name) 
            file_extension = ".txt"
            new_file_name = f"{file_name_without_extension}_{chunk_id}{file_extension}"
            new_file_path = os.path.join(folder, new_file_name)
            with open(new_file_path, 'w') as file:
                file.write(chunk_details.content)
            # c = canvas.Canvas(new_file_path, pagesize=letter)
            # width, height = letter

            # # Assuming the chunk_details.content is a list of strings
            # # Each string will be written as a new line in the PDF
            # lines = chunk_details.content.splitlines()
            # y = height - 10  # Start from the top of the page
            # for i, line in enumerate(lines):
            #     # Wrap the text to fit within the page width
            #     wrapped_text = textwrap.wrap(line, width=100)
            #     for j, wrapped_line in enumerate(wrapped_text):
            #         c.drawString(10, y - (j * 15), wrapped_line)
            #         y -= 15  # Move down by 15 units
            #     y -= 15 
            #     # Create a new page if the current y-coordinate is less than a certain threshold
            #     if y < 50:  # Change this value as needed
            #         c.showPage()
            #         y = height - 10  # Reset the y-coordinate
...
if __name__ == "__main__": 
    ...
    for index_config in config:
        ...
        end_time = time.time()  # Step 3: Capture the end time
        execution_time = end_time - start_time  # Step 4: Calculate the execution time
        print(f"Data preparation script finished in {execution_time} seconds")  # Step 5: Print the execution time
